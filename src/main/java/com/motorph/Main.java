...
public class Main {
    ...
    //------------------------------------------------------------------------------------------------------------------------------
    // STEP 1 — MAIN METHOD
    // Entry point. All CSV data is already loaded by the static initializers above.
    // Launches the login menu, which controls the full application lifecycle.
    //------------------------------------------------------------------------------------------------------------------------------
    ...
    //
    // Flow:
    //   1. Display logo and prompt for username and password.
    //   2. Valid usernames: "employee" or "payroll_staff". Password: 12345.
    //   3. If credentials are INCORRECT → display error and terminate.
    //   4. If credentials are CORRECT:
    //        - "payroll_staff" username → route to payrollStaffAccess()  (Step 2B)
    //        - "employee" username      → route to employeeAccess()       (Step 2A)
    //
    // Credentials are generated dynamically from the employee CSV roster
    // so that no passwords are hardcoded in the source file.
    // Username = full name lowercased with spaces removed.
    // Password = same string with the numeric employee ID appended.
    //------------------------------------------------------------------------------------------------------------------------------
  ...
  //------------------------------------------------------------------------------------------------------------------------------
    ...
    // Flow after login:
    //   Menu options:
    //     g → Display own monthly payroll breakdown (all months on record, June–December)
    //     l → Logout and return to login menu
    //
    //   The employee's own ID is passed in directly from the login result.
    //   They never enter an ID manually, preventing access to other employees' records.
    //
    //   Payroll display per month shows:
    //     1st Cutoff (Days 1–15) : Employee Number, Name, Birthday, Hours, Gross, Net
    //     2nd Cutoff (Days 16–End): Hours, Gross, each deduction itemized, Net
    //------------------------------------------------------------------------------------------------------------------------------
    ...
    // Flow after login:
    //   Main menu:
    //     g → Process Payroll — choose one employee or all employees
    //     a → Show raw attendance log (date, time-in, time-out) for selected employee
    //     e → Back to employee roster / logout
    //
    //   Process Payroll — One employee:
    //     1. Prompt for employee number.
    //     2. If not found → display "ID not found in employees".
    //     3. If found → display all months (June–December), per-cutoff:
    //          1st cutoff: Employee Number, Name, Birthday, Hours, Gross, Net
    //          2nd cutoff: Hours, Gross, SSS, PhilHealth, Pag-IBIG, Tax,
    //                      Total Deductions, Net
    //
    //   Process Payroll — All employees:
    //     Same format as above, repeated for every employee in the roster.
    //
    // An inner loop keeps staff inside a single employee's record until they
    // choose to go back, so each action does not force a return to the roster.
    //------------------------------------------------------------------------------------------------------------------------------
    ...
    static void displayLogo() {
        ...
        // ASCII art logo with alternating BLUE/RED segments to match MotorPH branding colors.
    ...
    static void displayIntro(HashMap<Integer, String> employees) throws IOException {
        ...
        // Table header for the employee roster shown to payroll staff.
        System.out.println("-".repeat(100));
    ...
    static void displayAttendance(HashMap<Integer, List<String>> dateMap, List<String> inList, List<String> outList, List<String> dateList, int chosenID) throws IOException {
        // Only display the attendance table if the employee has records on file.
    ...
    static void displayMonthlySalary(List<String> dateList, List<String> inList, List<String> outList, double HR) throws IOException {
        ...
        // Covers all available months in the attendance data (June through December).
        HashMap<Integer, long[]> monthSeconds = buildMonthSeconds(dateList, inList, outList);
        ...
        for (int month : monthSeconds.keySet()) {
            ...
            // ── 1st Cutoff (Days 1–15): gross only, no deductions ──
            ...
            // ── 2nd Cutoff (Days 16–End): gross minus all government deductions ──
            ...
            // ── Deduction breakdown for the 2nd cutoff ──
    ...
    static double computeNetGrossSalary(double monthlyGross) throws IOException {
        ...
        // This is the only cutoff where deductions are taken, per standard PH payroll practice.
...
//        System.out.println("totalContribution: " + totalContribution);
    ...
    static Map<String, HashMap<Integer, String>> parseEmployeeData() throws IOException {
        ...
        // Bundle all maps under a single return value to keep the method signature clean.
    ...
    static Map<String, HashMap<Integer, List<String>>> parseEmployeeAttendance() throws FileNotFoundException, IOException {
        ...
        // Read daily attendance logs. Each row represents one day's clock-in/out for one employee.
        ...
        while ((columnRead = attendanceReader.readLine()) != null) {
            ...
            // putIfAbsent initializes a new list for each new employee ID encountered,
            // so we never have to null-check before adding to the list.
    ...
    // Used to validate ID input before calling Integer.parseInt, preventing NumberFormatException crashes.
    ...
    static long secondsBetweenLog(int inHour, int inMinute, int outHour, int outMinute) {
        ...
        //Cap at 8 hours (28800 seconds) — prevents overtime from inflating computed pay.
